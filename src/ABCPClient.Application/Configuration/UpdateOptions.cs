...
public sealed class UpdateOptions
{
    ...
    /// </summary>
    /// <remarks>
    /// Репозиторий проекта публичный, поэтому значение пустое: релизы и их файлы
    /// читаются без авторизации. Поле оставлено на случай переезда в приватный
    /// репозиторий. Заполненное значение хранится в базе зашифрованным через DPAPI,
    /// как и реквизиты API, и в раздаваемый файл приложения не попадает: токен даёт
    /// доступ к репозиторию, а исполняемый файл может оказаться у кого угодно.
    /// </remarks>
